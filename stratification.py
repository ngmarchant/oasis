...
def bin_width_with_freedman_diaconis_rule(obs):
...
def stratify_equal_score(scores):
    width_score = bin_width_with_freedman_diaconis_rule(scores)
    goal_n_strata = np.ceil(np.ptp(scores) / width_score).astype(int)
    ...
    strata_populations = np.repeat(quotient, goal_n_strata) +\
                         np.concatenate((np.ones(remainder), np.zeros(goal_n_strata-remainder))).astype(int)

    start = 0
    for i, p in enumerate(strata_populations):
        allocations[sorted_ids[start: start+p]] = i
        start += p
...
if __name__ == '__main__':
    ...
    #print(bin_width_with_freedman_diaconis_rule(samples))
    print(samples)
    import matplotlib.pyplot as plt
    fig, ax = plt.subplots(1, 1)
    ax.hist(samples)
    plt.show()
    print(stratify_equal_score(samples))
...
def stratify_by_scores(scores, goal_n_strata='auto', method='cum_sqrt_F',
    ...
    if (method == 'cum_sqrt_F') or (goal_n_strata == 'auto'):
        ...
        if n_bins == 'auto':
            ...
            n_bins = np.ceil(np.ptp(scores)/width_score).astype(int)
        ...
        if goal_n_strata == 'auto':
            ...
            goal_n_strata = np.ceil(np.ptp(csf)/width_csf).astype(int)
